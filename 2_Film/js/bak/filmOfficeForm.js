// Shazia Pathan
/*
TOdo: verify clearing out "other values and anyrthing that disappears..
    validattions on blur.. i removed most... but what do the others do."
*/
...
var projectTitleValidators = {
  ...
  phoneValidators = {
    ...
    validators: {
      ...
      regexp: {
        regexp: /^[\s()+-]*([0-9][\s()+-]*){7,25}(?:[\-\.\ \\\/]?(?:#|ext\.?|extension|x)[\-\.\ \\\/]?(\d+))?$/i,
  ...
  episodeNoValidators = {
    ...
    validators: {
      ...
      notEmpty: {
        message: 'Episode number value is required'
      },
      callback: {
        callback: function(value, validator) {
          var episodeNo = parseInt(value);
          var episodes = parseInt($('#HowManyEpisodes').val());
          if (episodeNo > episodes) {
            return {
              valid: false,
              message: 'Episode number cannot be higher than total episodes',
            };
          } else {
            return true;
          }
...
function init() {
  //var strCode = '<link rel="stylesheet" href="/City%20Of%20Toronto/Sandbox/Steve/css/film.css">';
  var strCode = '<link rel="stylesheet" href="/City%20Of%20Toronto/Sandbox/Steve/css/film.css">';
...
function initForm() {
  ...
  $('#' + idForm).bootstrapValidator({
    ...
    fields: {
      ...
      'episodeNumber': episodeNoValidators,
    ...
    //$('#' + idForm).bootstrapValidator('revalidateField', 'TorontoSpendAmount');

  }).on('blur', '.moneyField', function(e) {
      //console.log(e.target.name + $("#" + e.target.name).val() + " - " + $("#" + e.target.name)[0].value)
  ...
  }).on('change', '[name="HowManyEpisodes"]', function(e) {
    $('#' + idForm).bootstrapValidator('revalidateField', 'episodeNumber');
  });
  /* sml: I don;t believe this is needed
  .on('success.field.fv', function(e, data) {
    if (data.field === 'OtherPlatform') {
      var platform = $('#' + idForm).find('[name="BroadcastPlatform"] option:selected').val();
...
function drawForm() {
  //$(displaySelector).load('html/pdfForm.html/pdfForm.html', function() {
  $(displaySelector).load('/City%20Of%20Toronto/Sandbox/Steve/html/pdfForm.html', function() {
    initForm();
  });
}
...
function dominoSubmit() {
  ...
  if ($("#ProductionType").val()== "Mini Series" || $("#ProductionType").val() == "Series" || $("#ProductionType").val() == "Reality") {
	...
	strBody += "<br><strong>Episode Number(s):</strong> " + $("#episodeNumber").val();
  } else {
	strBody += "<br><strong>Number of Episodes:</strong>";
	strBody += "<br><strong>Episode Number(s):</strong> ";
  }
  ...
  hf.setAttribute("value", "[http://www1.toronto.ca/wps/portal/contentonly?vgnextoid=95eb68bbb7e72510VgnVCM10000071d60f89RCRD&appInstanceName=default]");
  form.appendChild(hf);

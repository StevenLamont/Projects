...
function updateFilter() {
 ...
 //if (strQP == strQP2) {$("#resetFilters").css("display", "none");} else {$("#resetFilters").css("display", "");}
    if (Object.keys(gblURLParms).length === 0) {$("#resetFilters").css("display", "none");} else {$("#resetFilters").css("display", "");}
    var blnPrice = false;
    ...
    $.each(gblRestData.restaurants, function(i, item) {
        if ((qsVEG===""||item.lic_veggie===qsVEG)&&(qsVEGAN===""||item.lic_vegan==qsVEGAN)&&(qsLOC===""||item.lic_local==qsLOC)&&(qsACC===""||item.lic_accessible==qsACC)&&(qsCS===""||item.lic_cuisine.indexOf(qsCS)>=0)&&(qsNBH===""||item.lic_neighbourhood.indexOf(qsNBH)>=0) && ((qsLPA=="FALSE"&&qsLPB=="FALSE"&&qsLPC=="FALSE")||(item.lic_lunchprice.indexOf(qsLPA)>=0||item.lic_lunchprice.indexOf(qsLPB)>=0||item.lic_lunchprice.indexOf(qsLPC)>=0))&&((qsDPA=="FALSE"&&qsDPB=="FALSE"&&qsDPC=="FALSE")||(item.lic_dinnerprice.indexOf(qsDPA)>=0||item.lic_dinnerprice.indexOf(qsDPB)>=0||item.lic_dinnerprice.indexOf(qsDPC)>=0))) {
...
function drawDetail(strID) {
    ...
    //Show Modal before drawing map. If modal is already open, just redraw map. User map click on map icon in modal.

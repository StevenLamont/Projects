...
cot_form.prototype.render = function(o) {
	...
	$('#' + this.id).bootstrapValidator({
		...
		onError: function(e) {
			$($(".has-error input,.has-error select,.has-error textarea")[0]).focus();
		},
		fields: oVal.fields
	})
}
...
cot_form.prototype.processGrid = function(oRow, oVal, row) {
	...
	var oTableHeader = oTable.appendChild(document.createElement('thead'));
	var oTR = oTableHeader.appendChild(document.createElement('tr'));	
	...
	var oTableBody = oTable.appendChild(document.createElement('tbody'));
	oTR = oTableBody .appendChild(document.createElement('tr'));
	oTR.id = row.id + '-row-1';
...
cot_form.prototype.processField = function(oRow, oVal, row, field) {
	...
	if (field.type!='html'&&field.type!='button') {
		if (field.type=='static') {
			...
			oLabel.textContent = field.title;
		} else {

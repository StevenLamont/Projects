...
"0"!==g.ChristmasTree&&(c+="<img src='"+h.ctree+"' alt='Christmas Tree' title='Christmas Tree'>",f+="<span class='iconlbl'>Tree</span>");c+="</p>";c+=f}});""===c?(c="<p class='listingerr'>No collection schedule was found ("+e+")</p>",$("#colCal").hide()):($("#colCal").show(),c="<p class='listinghdr'>The next curbside collection for "+$("#searchLocation").val()+" is on:</p>"+c,$("#calPDFLink").attr("href","/City%20Of%20Toronto/Solid%20Waste%20Management%20Services/2%20Houses/Collection%20Calendar/Calendars/Daytime_"+
e.replace(/\s/g,"_")+".pdf"),$("#pdficon").attr("alt","Daytime_"+e.replace(/\s/g,"_")+".pdf"),$("#calPDFId").text("Daytime_"+e.replace(/\s/g,"_")+".pdf"),$("#calICSLink").attr("href","/City%20Of%20Toronto/Solid%20Waste%20Management%20Services/2%20Houses/Collection%20Calendar/.ICS%20Files/Waste Collection ("+e+").ics"),$("#icsicon").attr("alt","Daytime_"+e.replace(/\s/g,"_")+".ics"),$("#calICSId").text("Daytime_"+e.replace(/\s/g,"_")+".ics"));$("#calendar").show();$("#calendarData").html(c)}function y(a){m=
a.MasterCalendar.elements;t()}function z(a,b,d){$("#map").show();l=new google.maps.Map(document.getElementById("map-canvas"),{zoom:18,mapTypeId:google.maps.MapTypeId.ROADMAP,mapTypeControl:!0,mapTypeControlOptions:{style:google.maps.MapTypeControlStyle.DEFAULT,position:google.maps.ControlPosition.TOP_RIGHT},zoomControl:!0,draggable:!1});a=new google.maps.LatLng(a,b);var c=new google.maps.Marker({position:a,map:l,icon:"//maps.google.com/mapfiles/kml/pal2/icon13.png",title:d});l.addListener("center_changed",
function(){k.setTimeout(function(){l.panTo(c.getPosition())},1E3)});l.setCenter(a)}function u(a){z(a.lat,a.lng,a.addr);gisServerCheck.getServer("COT_GEOSPATIAL21_MTM").then(function(b){b="<HOST>/3/query?where=&text=&objectIds=&time=&geometry=<LNG>%2C<LAT>&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelWithin&relationParam=&outFields=AREA_ID%2CAREA_ATTR_ID%2CPARENT_AREA_ID%2CAREA_SHORT_CODE%2CAREA_NAME%2CAREA_DESC&returnGeometry=false&maxAllowableOffset=&geometryPrecision=&outSR=3857&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&returnTrueCurves=false&resultOffset=&resultRecordCount=&f=pjson".replace("<HOST>",
b).replace("<LNG>",a.lng).replace("<LAT>",a.lat);$.ajax({type:"GET",url:b,context:this,dataType:"jsonp",timeout:2500,success:function(a){e=a.features[0].attributes.AREA_NAME;$("#calendarId").text(e);null===m?Tabletop.init({key:"https://docs.google.com/spreadsheets/d/1Om0nwrYzeombeuMf-1pMksyG7oaTdXVpN3vR7-qrjdo/pubhtml",callback:y,simpleSheet:!1}):t()},error:function(a,b,e){bootbox.alert("Sorry, we can't determine the pickup schedule at this time.");$("#calendar").hide()}})})}function v(){n=new AddressLookup({matchType:1,

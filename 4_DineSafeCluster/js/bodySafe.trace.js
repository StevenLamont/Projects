...
3) rem,ove onClick events on infowindows so I can do the self-defined function
...
function createInfoWindow(KMLEvent_latLng, infoData) {
    ...
    //} else {
		...
		content += "<div class='row infoRow'><div class='col-md-12'><img src='" + statusIcons[infoData.establishments[i].status].medium + "'>&nbsp;&nbsp;<span class='infoImportant orgLink'><a href='#' data-id='" + infoData.establishments[0].estId + "'>" + infoData.establishments[i].name + "</a></span></div></div>";
        }
        content += "<div class='row infoRow'><div class='col-md-10 svLink'><a  href='#' data-latlng='" + KMLEvent_latLng + "'><img src='" + STREET_VIEW_ICON + "' alt='StreetView'></a></div></div>";
...
function mergeLocations(data) { /*ignore parms*/
    console.log('mergeLocations(data)  ');
    var locations = {};
...
function clearMarkers(markers) { /*ignore parms*/
    console.log('clearMarkers(markers)  ');
...
function plotMarkerShells(data) { /*ignore parms*/
    console.log('plotMarkerShells(data)  ');
    gblMapMarkerShells = [];
    var locations = mergeLocations(data);
    Object.keys(locations).forEach(function(key, idx, array) {
    //for( var i = 0; i < data.length; i++ ) {
...
function loadShellMarkerData() {
    console.log('loadShellMarkerData() ');
...
function setCluster(cluster, markers, flag) { /*ignore parms*/
    console.log('setCluster(cluster, markers, flag)  ');
...
function generateNumericPage() {
    console.log('generateNumericPage() ');
...
function setPagingMode(mode, startLetter) {
    console.log('setPagingMode(mode, startLetter) ' + mode +  ' - ' +  startLetter );

var codeSelector = '#appCodeComplaint';
var displaySelector = '#appDisplayComplaint';
var formSelector = "#complaintForm";
...
function dominoSubmit() {
  ...
  hf.setAttribute("value", "DineSafeComplaint");
  form.appendChild(hf);
  hf = document.createElement("input");
...
function  setupEvents() {
  ...
  $( formSelector ).submit(function( event ) {
    ...
    $("#complaintSubmission").on('keyup', function() {
...
function drawForm() {
    ...
    if (document.location.hostname.length === 0) {
        $(displaySelector).load('html/complaintForm.html', function() {
            initForm();
        }) ;
    } else {
        $(displaySelector).load('/static_files/WebApps/Health/DineSafe/html/complaintForm.html', function() {
            initForm();
        }) ;
    }
}

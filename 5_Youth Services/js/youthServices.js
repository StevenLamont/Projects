...
//https://drive.google.com/file/d/0B-j2Y49nfiw2bjZqOGgtcmJZbGs/view?usp=sharing
...
var markerImages = { single : "/static_files/WebApps/images/single.png",
                     fav : "/static_files/WebApps/images/favourite.png",
                     multi : "/static_files/WebApps/images/multi.png",
                     selected : "/static_files/WebApps/images/centre.png",
                     nolatlng : "/static_files/WebApps/images/marker_dot_multi.png",
                     listingMarker : "/static_files/WebApps/images/single.png",
                     foundMarker : "/static_files/WebApps/images/marker_sel_small.png",
                     findMe : "/static_files/WebApps/images/marker_sel.png"
                    };
var accImages = { F: { img: "/static_files/WebApps/images/fullyaccessibleicon.png",  title : "Fully Accessible"},
...
function highlightMapMarker(fid) {
    if (mapMarkers[fid]) {
        createSelectedMarker(mapMarkers[fid]);
        //mapMarkers[fid].setIcon(markerImages.selected);
        map.setZoom(15);
        switchToTab("tabMap");
...
function createInfoWindow(latLng, item) {
    ...
    content += "<div class='row infoRow'><div class='col-md-10'>";
    ...
    content += "<div class='row'><div class='col-xs-12 col-sm-12 col-md-12'>";
    if (favs[item.fid]) {
...
/* if you select an optiongroup, it is not selected in the multiselect  
   For each optiongroup there is a fake/hidden option. The the total is one more, unless there is only one option.
*/
function recheckTopicFilter() {
    ...
    Object.keys(gblTopicMap).forEach(function(topicId, idx, array) {
        var opts;
        if (gblTopicMap[topicId].parent === "" ) {      
            opts = gblTopicMap[topicId].options ;//+ 1;
        } else {
            opts = 1;
        }
    ...
    Object.keys(tMap).forEach(function(topicId, idx, array) {
        ...
        if (tMap[topicId].parent === "") {
            if (tMap[topicId].cnt === tMap[topicId].totalOptions) {
                ...
                //cntIncludingOptGroup += tMap[topicId].cnt + 1;
            } else {
                chk.first().prop('checked', false);
                chk.first().closest("li").removeClass("active");
                //cntIncludingOptGroup += tMap[topicId].cnt
            }
        }
     });
     //alert (cntIncludingOptGroup);
}
...
function addTopicOptions(data) {
    ...
    $.each(topicData, function(i,item) {
        ...
        if (item.parent === "") {
            ...
            if  (objectPropertyHasValue(item.children)) {
                ...
                var childrenTopics = item.children.sort(dynamicSort("","sortOrder"));
                for (var j = 0; j < childrenTopics.length; j++) {
                    
                    var subItem = childrenTopics[j];
                    if (subItem.parent !== "") {
                        childrenTopics[j].options = 0;
                    }
                    var newSubItem = {};
                    ...
                    if  (objectPropertyHasValue(subItem.children)) {
                        //newSubItem.submenu = {};
                        newSubItem.nodes = [];
                        var subChildrenTopics = subItem.children.sort(dynamicSort("","sortOrder"));
                        for (var k = 0; k < subChildrenTopics.length; k++) {
                            var subSubItem = subChildrenTopics[k];
                            var newSubSubItem = {};
    ...
    /* suboption will appear as children under parents, but are also repeated as themselves for quick access in the hash*/
    if (item.parent !== "") {
       item.options = 1;
    }
        ...
        title: function () {
            ...
            if (value && value.indexOf('all') === -1) {
                if(gblTopicMap[value] && gblTopicMap[value].tooltip) {
                    return gblTopicMap[value].tooltip;
                } 
            } else {
...
function setUpSearchFilters() {
    ...
    if (objectPropertyHasValue(window.$.QueryString.srchstr)) {
        ...
        ss.val(window.$.QueryString.srchstr);
        ss.next('span').css("display","block");
    } else {
        ss.next('span').css("display","none");
    }
...
function toggleFavs(ele, mode) {
    ...
    if (ele.hasClass("rem-fav")) {
        ...
        ele.html('<span class="glyphicon glyphicon-star" aria-hidden="true"></span> Add');
         ...
         if (listimgele.length > 0) {
            listimgele.prop("src", markerImages.single);
         }
            
    } else {
        if (Object.keys(favs).length >= MAX_FAVS && mode !== "syncList") {
    ...
    /* if in an infowindo, update listing */
     if (loc === "infw") {
         var listele = $( "#ysListing [data-id='" + fid + "']" );    //notice data-id and not fid
         if (listele.length > 0) {
            toggleFavs(listele, "syncList");
         }
     }
     saveFavourites();
     saveURLState();
...
function closeDetailWindow() {
    //$("a[data-id=80668]").get(0).scrollIntoView();
    var orgId = $("#detailModal").find("#org-fid").text();
    //$("a[data-id='" + orgId + "']").get(0).scrollIntoView();
    //$("#ys_" + orgId ).scrollTop( );
    ...
    if (gblCurrentTab === "tabList")  {
        $("#ys_" + orgId ).find("span").get(0).scrollIntoView();		
		//$("#ys_" + orgId ).find("span").focus();
    }
}
function setUpEvents() {
   ...
   $("#toggleShowFavs").on("click",function() {
        ...
        if ($(this).html().indexOf('Favourites') > -1) {
            $(this).html('<span class="glyphicon glyphicon-ok" aria-hidden="true"></span>&nbsp;&nbsp;Show Locations');
        } else {
 ...
 //   });
    ...
    $('#findMe').on("click", function() {
        ...
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            map.setZoom(18);
             if (gblFindMeMarker) {
                gblFindMeMarker.setMap(null);
            }
            gblFindMeMarker = new google.maps.Marker({
                position: initialLocation,
                map: map,
                icon: markerImages.findMe,
                zIndex: 0,
                title: "My Location"
            });         
            
            }, function() {
                bootbox.alert( {size: 'small', message : "Location Services is unavailable."});
            });
    ...
    $('#ttcBtn').on("click", function (event) {
            var orgAddr = $(".modal-body").find("#org-address").text();
            window.open(TTCPlanner + orgAddr, "_blank");
...
function initMap() {
    ...
    var findMeBTN = document.getElementById('findMe');
    map.controls[google.maps.ControlPosition.LEFT_TOP].push(findMeBTN);
    ...
    map.addListener('bounds_changed', function() {
          searchBox.setBounds(map.getBounds());
           //resetBTN.setBounds(map.getBounds());
    });
...
function updatePage() {
    ...
    plotMarkers();
    $("#ysListingRows").html("").trigger('update');
    gblLastLoaded = 0;
    showListing();
    $("#loader").hide();
    saveURLState();
...
function showListing() {
    ...
    $.each(gblFilteredData, function(i, item) {
        ...
        if (gblLastLoaded === 0) {
            strRows += "<div id='ys__" + item.fid + "'class='ystablerow'>&nbsp;</div>";
        }
        ...
        var markerIconSpan = "";
        if (objectPropertyHasValue(item.lat)) {
            markerIconSpan = "<span>&nbsp;&nbsp;&nbsp;<img title='Click to see location on main map' data-fid='" + item.fid + "' class='listingMarker' src='" + markerImg + "'>" + "</img><span>";
        }
        strRows += "<div id='ys_" + item.fid + "' class='ystablerow'>";
        strRows += "<div class='row'><div class='col-xs-9 col-sm-9 col-md-10'>";
        ...
        if ( objectPropertyHasValue(item.ph) || objectPropertyHasValue(item.web)) {
         ...
         strRows += "<div class='row'><div class='col-xs-9 col-sm-9 col-md-10'>&nbsp;</div></div>";
         var topics = decodeTopics(item.tps);
        //var topics = ;
        if (topics !== "") {
        ...
        if (typeof item.ens  !== "undefined") {
            strRows += "<div class='row'><div class='col-md-12'><span class='detailLabel'>Eligibility: </span>" +item.ens + "</div></div>";
        }
        ...
        */
        if (typeof item.sls  !== "undefined" ) {
            strRows += "<div class='row'><div class='col-md-12'><span class='detailLabel'>Languages: </span>" +item.sls + "</div></div>";
        }
        strRows += "</div>";
        strRows += "<div class='col-xs-3 col-sm-3 col-md-2'>" ;
            strRows += "<div class='row'><div class='col-xs-12 col-sm-12 col-md-12' style='padding-left: 0px; padding-right: 0px;'>";
            if (favs[item.fid])  {
    ...
    if (gblLastLoaded === 0) {
       strRows += "<div id=ysNoRows class='ystablerow'>No Rows Selected</div>"; 
    }
    $("#ysListingRows").append(strRows);
...
function loadMainPage() {
    ...
    if (document.location.hostname.length === 0 || document.location.hostname === 'localhost') {
        markerImages = { single : "images/single.png",
                        fav : "images/favourite.png",
                        multi : "images/multi.png",
                        selected : "images/centre.png",
                        nolatlng : "images/marker_dot_multi.png",
                        listingMarker : "images/single.png",                        
                        foundMarker : "images/marker_sel_small.png",
                        findMe : "images/marker_sel.png"
                        };
        ...
        strCode += '<script type="text/javascript"  src="js/pdfmake.min.js"></script>';
        ...
        strCode += '<script type="text/javascript"  src="js/pdfmakeHTMLConverter.min.js"></script>';
        ...
        strCode += '<link rel="stylesheet" href="/static_files/WebApps/Youth Services/css/youthServices.css">';
        strCode += '<link rel="stylesheet" href="/static_files/assets/multiselect/bootstrap-multiselect.css">';
        strCode += '<script type="text/javascript" src="/static_files/assets/multiselect/bootstrap-multiselect.js"></script>';
        strCode += '<script type="text/javascript" src="/static_files/WebApps/Youth%20Services/js/pdfmake.min.js"></script>';
        strCode += '<script type="text/javascript" src="/static_files/assets/pdfmake/vfs_fonts.js"></script>';
        strCode += '<script type="text/javascript" src="/static_files/WebApps/Youth%20Services/js/pdfmakeHTMLConverter.min.js"></script>';
        ...
        htmlLoad = '/static_files/WebApps/Youth%20Services/html/YouthServices.html';
    }

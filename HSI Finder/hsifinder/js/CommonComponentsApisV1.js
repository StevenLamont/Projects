...
/* 1) do I resolve errors, or just do an alert..
      I reject the deferred and the calling program, can trap with a fail() if desired.
    ...
    2) The assumption is that the app logs the user In and that a cookie contains the SID.
       ...
       Perhaps we could have an option that is slightly different.  When the user logs in, we validate, then save the userid/password.
       On every update/delete we could call and create a new session, so the user is never time out in a session and then the session is alive as long the browser is alive.
    ...
    3) We are not fully using options here.. we pass a timeout parm, but I could pass the EventType as an Option, 
          and then use it instead of passing them in call patterns. but then this function wouldn't act like as singleton.    
    
    4) any failure should return back an object of {type: xxx, error: yyy}
...
CC_API.prototype.getRepoData = function(eventType, repo, status, start, limit) {
    ...
    this.checkSession().done(function(thisObj) {
        var parms = {};
        if (repo !== null) { parms.repo = repo; }
        if (status !== null) {parms.status = status;}
        if (limit !== null) {parms.limit = parseInt(limit,10);}  // has to be an integer        
        if (start !== null) { parms.start = parseInt(start,10);} // has to be an integer        
